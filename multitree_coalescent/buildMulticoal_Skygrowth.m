...
clade = {'subsampled','all', 'Alpha', 'Delta', 'Omicron'};
...
for i = 1 : length(fastafiles)
    ...
    for v = 1: length(clade)
        ...
        rate_shift_immi = [14/366:14/366:(max_date-datenum(adjusted_start_date))/366];

        local_clusters = unique(dat{2});

        f = fopen('multicoal_template.xml');
        g = fopen(['xmls/multicoal_updated_' clade{v} strrep(fastafiles(1).name, 'fasta','xml')],'w');
        while ~feof(f)
            line = fgets(f);
