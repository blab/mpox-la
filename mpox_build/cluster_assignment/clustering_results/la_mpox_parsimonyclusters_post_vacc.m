...
%%
is_kc = find(ismember(division, 'Los Angeles County'));
...
for i = 1 : length(nodenames)
    if contains(nodenames{i},'NODE')
        if strcmp(location{i},'Los Angeles County')
            fprintf(f, '%s\tLos Angeles County\n',nodenames{i});
        else
            fprintf(f, '%s\tNot Los Angeles County\n',nodenames{i});
        end
    end
end
fclose(f);
...
for j = 1 : length(onlyKC)
    if length(location{j})==1 && location{j}=="Los Angeles County"
        onlyKC(j) = true;
...
f = fopen('../clustering_results/la_county_clusters_post_vacc.tsv', 'w');
