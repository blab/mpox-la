...
for rep = 2:8
    for sc = 1 : length(sample_cutoff)
        ...
        % sampling_times = cell(length(local_clusters),1);
        % max_sampling_times = zeros(length(local_clusters),1);
        % all_sampling = zeros(0,0);
        % 
        % for a = 1 : length(sampling_times)
        %     sampling_times{a} = zeros(0,0);
        %     seqs = strsplit(local_clusters{a}, ',');
        %     for b = 1: length(seqs)
        ...
        f = fopen('../multitree_coalescent/multicoal_template.xml');
        g = fopen(sprintf('../simulations/xmls/simmulticoal_%d.xml', rep), 'w');
        ...
        while ~feof(f)
            ...
            if contains(line, 'insert_data')
                for lc = 1:length(local_clusters)
                    names = strsplit(local_clusters{lc}, ',');
                   % disp(names)
