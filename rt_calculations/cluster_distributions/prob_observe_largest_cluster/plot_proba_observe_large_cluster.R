...
df_proba_obs_large_clust <- readRDS('./results/proba_large_polytomy/df_proba_obs_large_clust_LA.rds')
...
plot_proba_large_clust <- function(df_proba_obs_large_clust){
  ...
  plt_proba <- df_proba_obs_large_clust %>% 
    ...
    scale_fill_gradientn(name = paste0('Probability to observe a cluster of a least size ', size_largest_polytomy),
    ...
    scale_x_continuous(name = 'Reproduction number R',
                       ...
                       expand = expansion(mult = c(0., 0.))) +
    theme_classic() +
    coord_cartesian(ylim = c(log(0.009), log(14))) +
